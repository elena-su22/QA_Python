...
def transition_to_scene(i,loctypes,hero,heroitem,beastlist,dangerbeastlist):
    print(f'Текущая локация: {i}')
    print(type (i))
    print("Переход на локацию")
    if int(i)==0:
        heroitem = startloc(hero,heroitem)
        print(heroitem)
        return [hero,heroitem]
    if int(i)!=0:
        # heroitem = {}
        hero,heroitem = location(int(i),loctypes,hero,heroitem,beastlist,dangerbeastlist)
        print(hero)
        return [hero,heroitem]
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            ...
            # if event.type == pygame.MOUSEBUTTONDOWN:
            #     if text_rect.collidepoint(event.pos):
            #         print(grid[player_pos[0]][player_pos[1]] )
            #         print(type(grid[player_pos[0]][player_pos[1]] ))
            #         if int(grid[player_pos[0]][player_pos[1]]) == 0:
            #             transition_to_scene(grid[player_pos[0]][player_pos[1]],loctypes,hero,heroitem,beastlist,dangerbeastlist)
            #         if int(grid[player_pos[0]][player_pos[1]]) != 0:
            #             transition_to_scene(grid[player_pos[0]][player_pos[1]],loctypes,hero,heroitem,beastlist,dangerbeastlist)
        screen.fill((0, 0, 0))
