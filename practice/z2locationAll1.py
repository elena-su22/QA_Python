...
def location(locname=locname1,eventlistx = eventlistforest, hero=hero1):
    ...
    for i in range(3):
        ...
        if event == 'Храм':
            ...
            print('Вы продвигаетесь через густой лес внимательно прислушиваясь и напряженно сжимая в руке бластер, но кажется здесь нет ничего, что могло бы представлять для вас угрозу.') if locname == 'forest' else None
            print('Вы осторожно пробираетесь через болото, ваши ноги с хлюпаньем погружаются во влажный мох на каждом шагу. Этот путь напряженный, но, к счастью, безопасный') if locname == 'swamp' else None
            print('Вы бежите через равнину. Ноги путаются в траве. Тишина вокруг давит, кажется что-то смотрит на вас из тумана. Вы сжимаете бластер вспотевшей рукой.') if locname == 'plain' else None
            continue
        elif event == 'Бой':
